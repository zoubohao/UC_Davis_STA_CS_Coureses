LIBNAME lab6 "/folders/myfolders/Lab6";
...
* output graph;
ods listing gpath='/folders/myfolders/Lab6/';
ods graphics / imagename="p1" imagefmt=png;
...
proc lifetest data=bone_marrow plots=survival(atrisk=0 to 2500 by 500); 
time t2*dfree(0);
strata g/trend test=(logrank TARONE PETO MODPETO 
FLEMING(0,1)    );
run;

* Stratefied test;
proc lifetest data=bone_marrow ; 
time t2*dfree(0);
strata g/group=z10  test=(logrank TARONE PETO MODPETO 
FLEMING(0,1)    ); 
run;
...
* base;
proc phreg data =bone_marrow plots(overlay)=(survival);
class g(desc);
model t2*dfree(0) = g;
run;

* adjust for patient age z1 and donor age z2;
proc phreg data =bone_marrow plots(overlay)=(survival);
class g(desc);
model t2*dfree(0) = g z1 z2;
run;
...
ods listing gpath='/folders/myfolders/Lab6/';
ods graphics / imagename="p9" imagefmt=png;
proc phreg data =bone_marrow plots(overlay)=(survival);
class g(desc);
model t2*dfree(0) = g z1 z2;
baseline covariates=covs out=base /rowid=z1;
run;
...
;
run;
proc lifetest data=q1 nelson  ; 
time t*c(1);
ods output ProductLimitEstimates=ProductLimitEstimates;
run;

data ProductLimitEstimates; 
set ProductLimitEstimates;
if censor ne 1;

* variance for KM for survival;
VKM=stderr**2;

* H(t) via KM;
HKM=-log(Survival);
VHKM=(1/survival**2)*VKM;
keep t survival VKM HKM VHKM;
proc print;
run;
