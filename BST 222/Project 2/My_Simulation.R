...
sim_cox<- function(lambda0)
{
  ...
  1.943*(1-A) - 0.17*(1-sex) + 0.22 * (1-icgr_bigger_10) + 
    0.5 * (1-stage) - 0.05 * (1-p_TACE) + 0.3*(1-chile_class)
  +0.56*(1-open_surgery) - 0.02 * (1-intra_trans) + 
    0.45 * (1-nonanatom_resection) + 0.15*(1-satellite_nodule)
  + 0.14 * (1-micro_vascular_invasion) + 1.01*(1-mcig_type) 
  + 0.2*(1-h_confirmed_c)+0.67 * (1-pringle_maneuver)
  -0.28*serum_alb_level + 0.0039*serum_alt_level + 0.002*(1-long_op_time))) + 1 
  time_new = c()
  censor = c()
  for (t in Time){
    if (t > 125){
  ...
  dataset = data.frame(sex = sex, age = age,liver_disease = liver_disease, rli_type = rli_type,
                       ...
                       serum_alb_level= serum_alb_level, serum_alt_level= serum_alt_level,
                       ...
                       h_confirmed_c=h_confirmed_c, icgr_bigger_10 = icgr_bigger_10,
                       time=time_new,
                       censor=censor)
  return(dataset)
}
...
### sever data 
severData = mydata[which(mydata$min_Severe_RLI==1),]
